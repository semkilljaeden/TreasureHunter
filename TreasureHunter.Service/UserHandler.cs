...
namespace TreasureHunter.Service
{
    ...
    {
        public BotActor Bot { get; private set; }
        ...
        {
            _otherInventoryTask = Task.Factory.StartNew(() =>Inventory.FetchInventory(OtherSID, Bot.ApiKey, SteamWeb));
        }
        ...
        /// The current trade.
        /// </value>
        public Trade Trade => Bot.CurrentTrade;

        /// <summary>
        /// Gets the log the bot uses for convenience.
        /// </summary>
        public ILog Log => LogManager.GetLogger(typeof(UserHandler));
        ...
        private void SendMessage(Action<string> messageFunc, string message, System.Timers.Timer timer, params object[] formatParams)
        {
            ...
            {
                Log.Error($"Error occurred when sending message.  Message: \"{message}\" \nException: {ex.ToString()} ");
            }
        }
        ...
        private void SendTradeMessageImpl(string message)
        {
            ...
            {
                Log.Warn($"'{message}' is longer than 100 chars, it will be trimmed.");
            }
